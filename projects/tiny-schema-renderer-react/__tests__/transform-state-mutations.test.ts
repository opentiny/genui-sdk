...
describe('transformStateMutations', () => {
  it('compiles a simple assignment into setState/setIn', () => {
    const result = transformStateMutations('() => { this.state.count = 5; }');
    expect(result).toContain('this.setState(prev => this.setIn(prev, ["count"], 5))');
  });
  ...
  it('compiles compound assignment using the previous value', () => {
    ...
    expect(result).toContain('this.setState(prev => this.setIn(prev, ["count"], prev["count"] + 1))');
  ...
  it('compiles array push mutation', () => {
    ...
    expect(result).toContain('this.setIn(prev, ["items"], [...prev["items"], "c"])');
  });

  it('keeps reads untouched', () => {
    const result = transformStateMutations('() => { return this.state.count + 1; }');
    expect(result).not.toContain('setState');
    expect(result).not.toContain('setIn');
