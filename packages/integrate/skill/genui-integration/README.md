# GenUI SDK Integration Skill

跨平台 Agent Skill，帮助用户在支持 Skills 规范的 AI 助手中集成 GenUI SDK。

## 概述

本 skill 提供完整的 GenUI SDK 集成指导，支持：

- **Vue 3** - 使用 `GenuiChat` 或 `GenuiRenderer`
- **Angular** - 使用 `GenuiRenderer`
- **Server** - Node.js 后端服务集成

## 安装

```bash
npm install @opentiny/genui-sdk-integration-skill
```

将 skill 目录指向安装后的包路径（含 `SKILL.md`），具体配置方式取决于你所用的 AI 助手平台。

## 快速开始

在 AI 助手中触发本 skill：

```
帮我集成 GenUI SDK 到 Vue 项目
如何在 Angular 中使用 GenuiRenderer？
怎么设置 genui-sdk-server？
```

## 文档同步

部分内容以本仓 `docs/src` 为源，构建时由 `scripts/copy-from-docs.mjs` 生成（**不入库**，发布时 `prepack` 会生成）：

| 路径 | 源 |
|------|-----|
| `examples/` | `docs/src/examples/` |
| `references/materials/` 专页（`vue-*.md` / `angular-*.md`） | `docs/src/components/materials/` |

本地开发：改 docs 后在本包执行 `npm run build`。  
`references/materials/index.md` 以及 `references/vue.md` / `angular.md` / `server.md` 由 skill 手工维护并提交。

## 相关资源

- [GenUI SDK 在线文档](https://docs.opentiny.design/genui-sdk/)
- [GenUI SDK GitHub](https://github.com/opentiny/genui-sdk)

## 许可证

MIT License
